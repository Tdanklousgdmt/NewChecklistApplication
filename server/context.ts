...
async function verifyViaSupabaseAuth(
  ...
  const anonKey = (apikeyHeader || process.env.SUPABASE_ANON_KEY || FALLBACK_SUPABASE_ANON_KEY || "").trim();
...
export async function verifySupabaseJwt(
  ...
  if (!secret) {
    console.warn("JWT secret not set; falling back to decoded authenticated payload for compatibility.");
...
export function toRequestContext(jwt: { sub: string; email: string }, profile: UserProfile): RequestContext {
  return {
    userId: jwt.sub,
